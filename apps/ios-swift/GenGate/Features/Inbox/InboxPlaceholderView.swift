...
struct InboxPlaceholderView: View {
    ...
    private var recipientDeviceSelectionSourceHintText: String? {
        ...
        guard !trimmedRecipientDeviceID.isEmpty else {
            if let firstRecipientDeviceOptionID {
                return "Recipient device source: chưa chọn (first option: \(firstRecipientDeviceOptionID), short: \(shortUserID(firstRecipientDeviceOptionID)))."
            }
        ...
        if recipientDeviceOptions.contains(where: { $0.id == trimmedRecipientDeviceID }) {
            return "Recipient device source: current options (in-sync, short: \(shortUserID(trimmedRecipientDeviceID)))."
        }

        return "Recipient device source: manual UUID/out-of-options (short: \(shortUserID(trimmedRecipientDeviceID)))."
    }
