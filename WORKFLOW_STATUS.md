...
- Scope: batch 186 iOS inbox seam hardening — add quick-copy for source-hint preview-pair `use_when` marker
- Status: verify
- Files:
...
- Git:
  - latest commit: `2950bde` — `batch186: add preview-pair use-when quick copy`
  - working tree: sạch (sau commit local, chưa push)
- Blocker: none
- Next: mở batch187 cho messaging friction tiếp theo (ví dụ thêm copy-ready `preview-pair-lite` block chỉ gồm `use_when + preview` để note siêu ngắn) trong iOS inbox shell
