...
- Scope: batch 322 feed shell (web+iOS) — persist/display `last_source_state_snapshot=...` token after snapshot quick-copy so QA can compare latest readiness payload without leaving UI.
- Status: complete
...
- Git:
  - latest feature commit: `6f8fcbd` — `batch322: persist last source-state snapshot token on web and ios`
  - previous feature commit: `d8a2309` — `batch321: add source-state snapshot quick copy on web and ios`
  - working tree: clean after batch322 feature + workflow sync commits
- Blocker: none
- Next: mở batch323 với 1 slice hẹp feed shell (web+iOS) — thêm quick-copy action riêng cho `last_source_state_snapshot=...` line để QA re-copy snapshot token mà không phải copy lại source-state raw line.
- Batch 295 handoff:
