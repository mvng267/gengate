...
- Scope: batch 319 feed shell (web+iOS) — add dedicated quick-copy action for `delete_copy_audit_first_ready_source=...` marker so QA can copy selected-source marker directly.
- Status: complete
...
- Git:
  - latest feature commit: `f876528` — `batch319: add first-ready source-line quick copy on web and ios`
  - previous feature commit: `84ab5b2` — `batch318: add first-ready source marker for delete copy audit on web and ios`
  - working tree: clean after batch319 feature + workflow sync commits
- Blocker: none
- Next: mở batch320 với 1 slice hẹp feed shell (web+iOS) — thêm source-state line `ready_count=<n>/total=<n>` để QA đọc nhanh số nguồn available trước khi chạy one-shot copy.
- Batch 295 handoff:
