...
- Scope: batch 315 feed shell (web+iOS) — add quick filter chips `delete_copy_audit_source` to force-generate `delete_copy_audit=source:.../value:...` from selected source without replaying click sequence.
- Status: complete
...
- Git:
  - latest feature commit: `e035bba` — `batch315: add delete copy audit source chips on web and ios`
  - previous feature commit: `bb09591` — `batch314: add delete copy audit quick-copy line on web and ios`
  - working tree: clean after batch315 feature + workflow sync commits
- Blocker: none
- Next: mở batch316 với 1 slice hẹp feed shell (web+iOS) — thêm `delete_copy_audit_source_state=ready|missing` summary line + copy action để QA biết trước source nào có payload hợp lệ trước khi bấm chip.
- Batch 295 handoff:
