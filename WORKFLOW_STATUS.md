...
- Scope: batch 314 feed shell (web+iOS) — add quick-copy audit token line `delete_copy_audit=source:.../value:...` + copy action so delete-summary copy provenance can be reported as one short payload.
- Status: complete
...
- Blocker: none
- Next: mở batch315 với 1 slice hẹp feed shell (web+iOS) — thêm `delete_copy_audit_source` quick filter chips (`quick_delete_parity` / `last_delete_result` / `copied_feedback`) để QA force-generate audit token theo từng source mà không phải nhớ thứ tự click.
- Batch 295 handoff:
