...
- Scope: batch 311 feed shell (web+iOS) — add `delete_snapshot_source=manual_input|preset_row|first_authored_quick_pick` marker into delete parity summaries for clearer delete-target provenance during human create->delete tests.
- Status: complete
...
- Git:
  - latest feature commit: `ea67e22` — `batch311: add delete snapshot source markers on web and ios`
  - previous feature commit: `079f731` — `batch310: add delete summary quick-copy actions on web and ios`
  - working tree: clean after batch311 feature + workflow sync commits
- Blocker: none
- Next: mở batch312 với 1 slice hẹp feed shell (web+iOS) — thêm quick-copy action riêng cho `Last copied delete summary`/`Copied delete summary` feedback token để QA có thể re-copy lại payload vừa copy mà không cần delete lại.
- Batch 295 handoff:
