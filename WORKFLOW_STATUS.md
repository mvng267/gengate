...
- Scope: batch 324 feed shell (web+iOS) — add source marker `last_source_state_snapshot_source=source_state_snapshot_copy|manual_recopy` so QA can distinguish fresh snapshot capture vs manual recopy from stored token.
- Status: complete
...
- Git:
  - latest feature commit: `927a297` — `batch324: add source markers for last snapshot recopy on web and ios`
  - previous feature commit: `e9fac07` — `batch323: add last source-state snapshot quick copy on web and ios`
  - working tree: clean after batch324 feature + workflow sync commits
- Blocker: none
- Next: mở batch325 với 1 slice hẹp feed shell (web+iOS) — thêm quick-copy action riêng cho line `last_source_state_snapshot_source=...` để QA copy marker nguồn snapshot nhanh trong report.
- Batch 295 handoff:
