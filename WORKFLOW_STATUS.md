...
- Scope: batch 320 feed shell (web+iOS) — append aggregate readiness counters `ready_count=<n>/total=<n>` into `delete_copy_audit_source_state=...` so QA can scan source availability faster before one-shot copy.
- Status: complete
...
- Git:
  - latest feature commit: `f7f16a2` — `batch320: add delete copy audit ready-count source-state on web and ios`
  - previous feature commit: `f876528` — `batch319: add first-ready source-line quick copy on web and ios`
  - working tree: clean after batch320 feature + workflow sync commits
- Blocker: none
- Next: mở batch321 với 1 slice hẹp feed shell (web+iOS) — thêm quick-copy action riêng cho source-state line đã có `ready_count` để QA copy snapshot readiness đầy đủ bằng 1 click.
- Batch 295 handoff:
