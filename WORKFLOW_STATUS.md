...
- Scope: batch 321 feed shell (web+iOS) — add dedicated quick-copy action for source-state snapshot line (already containing `ready_count`) so QA can copy full readiness snapshot in one click.
- Status: complete
...
- Git:
  - latest feature commit: `d8a2309` — `batch321: add source-state snapshot quick copy on web and ios`
  - previous feature commit: `f7f16a2` — `batch320: add delete copy audit ready-count source-state on web and ios`
  - working tree: clean after batch321 feature + workflow sync commits
- Blocker: none
- Next: mở batch322 với 1 slice hẹp feed shell (web+iOS) — thêm token `last_source_state_snapshot=...` lưu lại payload source-state vừa copy để QA đối chiếu snapshot history ngay trên UI.
- Batch 295 handoff:
