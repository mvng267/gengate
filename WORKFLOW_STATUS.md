...
- Scope: batch 325 feed shell (web+iOS) — add dedicated quick-copy action for `last_source_state_snapshot_source=...` line so QA can copy snapshot-source marker directly in reports.
- Status: complete
...
- Git:
  - latest feature commit: `09c7ee8` — `batch325: add snapshot source-line quick copy on web and ios`
  - previous feature commit: `927a297` — `batch324: add source markers for last snapshot recopy on web and ios`
  - working tree: clean after batch325 feature + workflow sync commits
- Blocker: none
- Next: mở batch326 với 1 slice hẹp feed shell (web+iOS) — thêm status marker `last_source_state_snapshot_source_line_copied` vào feedback khi copy source-marker line để QA scan log nhanh không cần mở clipboard.
- Batch 295 handoff:
