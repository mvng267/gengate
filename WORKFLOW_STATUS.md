...
- Scope: batch 316 feed shell (web+iOS) — add `delete_copy_audit_source_state=...` readiness line + copy action so QA can see ready/missing payload state per source before copy.
- Status: complete
...
- Git:
  - latest feature commit: `bbe504c` — `batch316: add delete copy audit source-state line on web and ios`
  - previous feature commit: `e035bba` — `batch315: add delete copy audit source chips on web and ios`
  - working tree: clean after batch316 feature + workflow sync commits
- Blocker: none
- Next: mở batch317 với 1 slice hẹp feed shell (web+iOS) — thêm quick action `Copy delete copy audit for first ready source` để QA lấy audit payload nhanh nhất khi chỉ cần one-shot report.
- Batch 295 handoff:
