...
- Batch workflow chính thức mới nhất trong checklist/status: **430 — web notifications parity now hardens quick-copy copied-feedback lines for unread/page/meta/mutation/create/lifecycle actions and clears them on window/create/mutation/delete refresh to avoid stale markers.**
...
- Scope hiện tại: web notifications parity — harden copied-feedback context lines cho quick unread/page meta/page cursor/mutation/create-result/lifecycle copy actions + clear context khi load/create/mutation/delete refresh.
- Trạng thái hiện tại: **complete**
- File đã đụng:
  - `apps/web-nextjs/components/notification-shell.tsx`
- Test-verify:
  - `cd apps/web-nextjs && npm run typecheck` → ✅ (`tsc --noEmit`)
  - `cd apps/backend-python && make test-friendships` → ✅ (`8 passed in 0.69s`)
- Git mốc gần nhất:
  - commit đã chốt:
    - `9375b17` — `batch430: harden web notifications quick-copy copied-feedback parity`
  - working tree hiện tại: dirty (workflow docs chưa commit)
- Blocker nếu có:
  - none.
- Bước kế tiếp:
  - mở batch431 với 1 micro-slice product seam (ưu tiên notifications/location/feed parity follow-up) + giữ verify tối thiểu `make test-friendships`.
