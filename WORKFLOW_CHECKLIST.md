...
- Batch workflow chính thức mới nhất trong checklist/status: **312 — feed shell (web+iOS copied-delete-summary feedback quick copy) đã complete**.
...
- Batch workflow chính thức hiện tại: **312**
- Scope hiện tại: feed shell (web+iOS) — thêm quick-copy action cho feedback token `Last copied delete summary`/`Copied delete summary` để QA re-copy payload vừa copy mà không cần delete lại.
...
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `29edaca` — `batch312: add copied-delete-summary feedback quick copy on web and ios`
  - commit liền trước: `ea67e22` — `batch311: add delete snapshot source markers on web and ios`
  - working tree hiện tại: sạch
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch313 với 1 slice hẹp feed shell (web+iOS): thêm marker `delete_summary_copy_source=quick_delete_parity|last_delete_result|copied_feedback` vào status copy success để audit nguồn copy action ngay trong log/status.
