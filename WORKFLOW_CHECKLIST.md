...
- Batch workflow chính thức mới nhất trong checklist/status: **134 — iOS inbox thread-identity timestamp-reset slice is in verify**.
...
- Scope hiện tại: iOS inbox read-status UX hardening — clear recipient-device timestamp debug state khi đổi identity User A/User B.
...
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `HEAD` (local batch134 slice)
  - working tree hiện tại: sạch (sau commit local, chưa push)
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch135 cho messaging friction tiếp theo (clear stale recipient-device options khi đổi identity thread) trong iOS inbox shell
