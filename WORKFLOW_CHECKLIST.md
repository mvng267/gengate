...
- Batch workflow chính thức mới nhất trong checklist/status: **398 — iOS inbox shell now aligns sender keep-pair quick-copy failure handling parity (`quick_copy_clipboard_unavailable` / `sender_keep_pair_quick_copy_failed`) with web.**
...
- Scope hiện tại: iOS inbox send quick-copy parity — align sender keep-pair quick-copy failure handling trong `InboxPlaceholderView`.
- Trạng thái hiện tại: **complete**
- File đã đụng:
  - `apps/ios-swift/GenGate/Features/Inbox/InboxPlaceholderView.swift`
- Test-verify:
  - `cd apps/ios-swift && swift build` → ✅ (`Build complete! (16.57s)`)
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `1d9114c` — `batch398: harden ios sender keep-pair quick-copy failures`
  - working tree hiện tại: clean
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch399 với 1 slice hẹp kế tiếp theo seam MVP (ưu tiên iOS inbox sender-keep-pair + send-bundle quick-copy failure token parity với web `sender_keep_pair_send_bundle_quick_copy_failed`).
