...
- Batch workflow chính thức mới nhất trong checklist/status: **113 — iOS inbox member-cursor message-target picker slice is in verify**.
...
- Batch workflow chính thức hiện tại: **113**
- Scope hiện tại: iOS inbox read-status UX hardening — quick row action sets read-cursor target message from member cursor.
...
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `HEAD` (local batch113 slice)
  - working tree hiện tại: sạch (sau commit local, chưa push)
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch114 cho messaging friction tiếp theo (one-tap apply member cursor user+message vào read-cursor form) trong iOS inbox shell
