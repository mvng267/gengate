...
- Batch workflow chính thức mới nhất trong checklist/status: **405 — iOS profile friend-graph shell now aligns last-action summary bundle quick-copy failure handling parity (`quick_copy_clipboard_unavailable` / `friend_request_last_action_bundle_quick_copy_failed`) with web.**
...
- Scope hiện tại: iOS friend graph quick-copy parity — align last-action summary bundle quick-copy failure handling trong `ProfilePlaceholderView`.
- Trạng thái hiện tại: **complete**
- File đã đụng:
  - `apps/ios-swift/GenGate/Features/Profile/ProfilePlaceholderView.swift`
- Test-verify:
  - `cd apps/ios-swift && swift build` → ✅ (`Build complete! (3.47s)`)
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `acd49c3` — `batch405: harden ios friend last-action bundle quick-copy failures`
  - working tree hiện tại: clean
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch406 với 1 slice hẹp kế tiếp theo seam MVP (ưu tiên hoàn tất các quick-copy failure token parity còn lại trong iOS friend graph hoặc backend/web friend-graph contract polish).
