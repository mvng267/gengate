...
- Batch workflow chính thức mới nhất trong checklist/status: **397 — iOS inbox shell now aligns send-result quick-copy failure handling parity (`quick_copy_clipboard_unavailable` / `send_result_quick_copy_failed`) with web.**
...
- Batch workflow chính thức hiện tại: **397**
- Scope hiện tại: iOS inbox send quick-copy parity — align send-result quick-copy failure handling trong `InboxPlaceholderView`.
- Trạng thái hiện tại: **complete**
- File đã đụng:
  - `apps/ios-swift/GenGate/Features/Inbox/InboxPlaceholderView.swift`
- Test-verify:
  - `cd apps/ios-swift && swift build` → ✅ (`Build complete! (26.81s)`)
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `ba5e9b4` — `batch397: harden ios send quick-copy failure tokens`
  - working tree hiện tại: clean
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch398 với 1 slice hẹp kế tiếp theo seam MVP (ưu tiên iOS inbox sender-keep-pair quick-copy failure token parity với web `sender_keep_pair_quick_copy_failed`).
