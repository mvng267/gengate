...
- Scope hiện tại: web inbox contract parity — ưu tiên backend `error.code` tokens trong direct-message client failures.
...
- Bước kế tiếp:
  - mở batch412 với 1 micro-slice iOS inbox parity đối xứng (ưu tiên error-code token handling tương ứng với web inbox client).
