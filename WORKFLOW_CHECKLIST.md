...
- Batch workflow chính thức mới nhất trong checklist/status: **323 — feed shell (web+iOS last source-state snapshot quick copy) đã complete**.
...
- Scope hiện tại: feed shell (web+iOS) — thêm quick-copy action riêng cho line `last_source_state_snapshot=...` để QA re-copy snapshot token nhanh mà không cần copy lại source-state raw line.
...
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `e9fac07` — `batch323: add last source-state snapshot quick copy on web and ios`
  - commit liền trước: `6f8fcbd` — `batch322: persist last source-state snapshot token on web and ios`
  - working tree hiện tại: sạch
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch324 với 1 slice hẹp feed shell (web+iOS): thêm source marker `last_source_state_snapshot_source=source_state_snapshot_copy|manual_recopy` để QA phân biệt token mới refresh vs re-copy từ snapshot đã lưu.
