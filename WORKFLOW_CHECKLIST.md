...
- Scope hiện tại: iOS moments/feed contract parity — ưu tiên backend `error.code` token thay vì chuỗi lỗi có prefix/status khi request fail.
- Trạng thái hiện tại: **complete**
- File đã đụng:
  - `apps/ios-swift/GenGate/Features/Feed/FeedPlaceholderView.swift`
- Test-verify:
  - `cd apps/ios-swift && swift build` → ✅ (`Build complete! (5.34s)`)
...
- Bước kế tiếp:
  - mở batch410 với 1 micro-slice moments/feed/backend contract follow-up nhỏ, giữ parity semantics web/iOS và dùng verify command chạy được trong env hiện tại.
