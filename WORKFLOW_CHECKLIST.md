...
- Scope hiện tại: moments/feed contract polish — normalize web moment client error-code tokens và surface feed error-hint parity ở web compose shell.
...
- Test-verify:
  ...
  - `cd apps/ios-swift && swift build` → ✅ (`Build complete! (0.18s)`)
...
- Bước kế tiếp:
  - mở batch409 với 1 micro-slice moments/feed parity kế tiếp (ưu tiên iOS error-token parity follow-up hoặc backend contract follow-up nhỏ), dùng verify command chạy được trong env hiện tại.
