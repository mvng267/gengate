...
- Batch workflow chính thức mới nhất trong checklist/status: **400 — iOS inbox shell now aligns read-cursor quick-copy failure handling parity (`quick_copy_clipboard_unavailable` / `read_cursor_quick_copy_failed`) with web.**
...
- Batch workflow chính thức hiện tại: **400**
- Scope hiện tại: iOS inbox read-cursor quick-copy parity — align read-cursor quick-copy failure handling trong `InboxPlaceholderView`.
- Trạng thái hiện tại: **complete**
- File đã đụng:
  - `apps/ios-swift/GenGate/Features/Inbox/InboxPlaceholderView.swift`
- Test-verify:
  - `cd apps/ios-swift && swift build` → ✅ (`Build complete! (14.97s)`)
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `477fe8f` — `batch400: harden ios read-cursor quick-copy failures`
  - working tree hiện tại: clean
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch401 với 1 slice hẹp kế tiếp theo seam MVP (ưu tiên iOS inbox read-cursor-apply quick-copy failure token parity với web `read_cursor_apply_quick_copy_failed`).
