...
- Batch workflow chính thức hiện tại: **275**
- Scope hiện tại: direct-message shell — thêm one-tap member-row action để set read-cursor target message từ selected member cursor (`last_read_message_id`) trên web+iOS.
...
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `(pending in this run)` — `batch275: add member-cursor message-target one-tap actions on web and ios`
  - working tree hiện tại: bẩn (2 files) trước commit
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch276 với 1 slice hẹp direct-message shell: thêm one-tap action apply đồng thời member user + member cursor message vào read-cursor target form trên web+iOS.
