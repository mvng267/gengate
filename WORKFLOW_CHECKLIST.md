...
- Batch workflow chính thức mới nhất trong checklist/status: **420 — web location parity now clears copied-state feedback when quick-copy fails or clipboard is unavailable.**
...
- Scope hiện tại: web location parity — clear copied location-state feedback marker when quick copy fails or clipboard is unavailable.
...
- Blocker nếu có:
  - none.
- Bước kế tiếp:
  - mở batch421 với 1 micro-slice product seam (ưu tiên notifications/location/feed parity follow-up) + giữ verify tối thiểu `make test-friendships`.
