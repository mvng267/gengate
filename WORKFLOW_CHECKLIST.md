...
- Batch workflow chính thức mới nhất trong checklist/status: **422 — iOS location parity now hardens audience-remove quick-copy with deterministic failure token and copied-feedback reset.**
...
- Scope hiện tại: iOS location parity — harden audience-remove quick-copy failure/unavailable semantics và reset copied-feedback state.
- Trạng thái hiện tại: **complete**
- File đã đụng:
  - `apps/ios-swift/GenGate/Features/Location/LocationPlaceholderView.swift`
- Test-verify:
  - `cd apps/ios-swift && swift build` → ✅ (`Build complete! (1.73s)`)
...
- Blocker nếu có:
  - none.
- Bước kế tiếp:
  - mở batch423 với 1 micro-slice product seam (ưu tiên notifications/location/feed parity follow-up) + giữ verify tối thiểu `make test-friendships`.
