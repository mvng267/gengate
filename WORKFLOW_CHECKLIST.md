...
- Batch workflow chính thức mới nhất trong checklist/status: **311 — feed shell (web+iOS delete snapshot source markers) đã complete**.
...
- Batch workflow chính thức hiện tại: **311**
- Scope hiện tại: feed shell (web+iOS) — thêm marker `delete_snapshot_source=manual_input|preset_row|first_authored_quick_pick` vào quick delete parity summary để trace nguồn delete target rõ hơn khi human test.
...
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `ea67e22` — `batch311: add delete snapshot source markers on web and ios`
  - commit liền trước: `079f731` — `batch310: add delete summary quick-copy actions on web and ios`
  - working tree hiện tại: sạch
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch312 với 1 slice hẹp feed shell (web+iOS): thêm quick-copy action riêng cho feedback token `Last copied delete summary`/`Copied delete summary` để QA re-copy payload vừa copy mà không cần delete lại.
