...
- Batch workflow chính thức mới nhất trong checklist/status: **141 — iOS inbox short recipient-id mismatch helper-note slice is in verify**.
...
- Scope hiện tại: iOS inbox read-status UX hardening — show helper note chi tiết mismatch kèm recipient user id rút gọn khi non-member auto-clear xảy ra.
...
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `HEAD` (local batch141 slice)
  - working tree hiện tại: sạch (sau commit local, chưa push)
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch142 cho messaging friction tiếp theo (truncate helper note reason string cho consistent width trên iPhone màn hình nhỏ, tránh line-wrap quá dài trong recipient-device section) trong iOS inbox shell
