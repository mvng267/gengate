...
- Batch workflow chính thức mới nhất trong checklist/status: **124 — iOS inbox stale attachment/device-key target guard slice is in verify**.
...
- Scope hiện tại: iOS inbox read-status UX hardening — clear stale manual attachment/device-key target message UUIDs when missing from loaded rows.
...
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `HEAD` (local batch124 slice)
  - working tree hiện tại: sạch (sau commit local, chưa push)
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch125 cho messaging friction tiếp theo (fallback recipientDeviceIDDraft when selected device no longer exists in refreshed recipient list) trong iOS inbox shell
