...
- Batch workflow chính thức mới nhất trong checklist/status: **421 — iOS location parity now clears quick location-state copied feedback on failures and emits deterministic copy-failure token.**
...
- Scope hiện tại: iOS location parity — clear quick location-state copied feedback marker on copy failure/unavailable paths.
- Trạng thái hiện tại: **complete**
- File đã đụng:
  - `apps/ios-swift/GenGate/Features/Location/LocationPlaceholderView.swift`
- Test-verify:
  - `cd apps/ios-swift && swift build` → ✅ (`Build complete! (2.53s)`)
...
- Blocker nếu có:
  - none.
- Bước kế tiếp:
  - mở batch422 với 1 micro-slice product seam (ưu tiên notifications/location/feed parity follow-up) + giữ verify tối thiểu `make test-friendships`.
