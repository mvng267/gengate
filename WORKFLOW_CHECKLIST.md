...
- Scope hiện tại: direct-message shell — thêm read-cursor apply-state marker `read_cursor_apply_state=noop|updated` trong quick-copy + status để tách rõ outcome no-op/update trên web+iOS.
...
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `(pending in this run)` — `batch284: add read-cursor apply-state markers on web and ios`
  - working tree hiện tại: sạch
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch285 với 1 slice hẹp direct-message shell: thêm quick-copy note cho last-read baseline (`previous_cursor_message`) để explain tại sao apply_state=noop trên web+iOS.
