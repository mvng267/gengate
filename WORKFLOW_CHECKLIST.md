...
- Batch workflow chính thức mới nhất trong checklist/status: **108 — iOS inbox quick read-cursor preset slice is in verify**.
...
- Batch workflow chính thức hiện tại: **108**
- Scope hiện tại: iOS inbox read-status UX hardening — quick preset read-cursor user/message chips.
...
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `HEAD` (local batch108 slice)
  - working tree hiện tại: sạch (sau commit local, chưa push)
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch109 cho messaging friction tiếp theo (inline last-read ordering hint) trong iOS inbox shell
