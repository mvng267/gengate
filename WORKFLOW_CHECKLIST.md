...
- Batch workflow chính thức mới nhất trong checklist/status: **429 — iOS notifications parity now resets quick-copy context bundles on load window refresh to avoid stale create/mutation/delete markers after window changes.**
...
- Scope hiện tại: iOS notifications parity — reset quick-copy context bundles khi load window đổi để tránh stale markers sau reload.
- Trạng thái hiện tại: **complete**
- File đã đụng:
  - `apps/ios-swift/GenGate/Features/Notifications/NotificationsPlaceholderView.swift`
- Test-verify:
  - `cd apps/ios-swift && swift build` → ✅ (`Build complete! (3.82s)`)
...
- Git mốc gần nhất:
  ...
  - working tree hiện tại: dirty (workflow docs đang cập nhật để sync batch429)
- Blocker nếu có:
  - none.
- Bước kế tiếp:
  - mở batch430 với 1 micro-slice product seam (ưu tiên notifications/location/feed parity follow-up) + giữ verify tối thiểu `make test-friendships`.
