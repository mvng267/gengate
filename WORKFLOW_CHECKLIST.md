...
- Batch workflow chính thức mới nhất trong checklist/status: **125 — iOS inbox recipient-device fallback guard slice is in verify**.
...
- Scope hiện tại: iOS inbox read-status UX hardening — recipient-device fallback guard khi selected device bị stale so với option list đã refresh.
...
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `HEAD` (local batch125 slice)
  - working tree hiện tại: sạch (sau commit local, chưa push)
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch126 cho messaging friction tiếp theo (auto reload recipient devices when recipient user changes with lightweight debounce) trong iOS inbox shell
