...
- Batch workflow chính thức mới nhất trong checklist/status: **425 — iOS location parity now resets stale quick-copy feedback when location state/audience context changes.**
...
- Scope hiện tại: iOS location parity — reset stale quick-copy feedback khi location state/audience context đổi.
- Trạng thái hiện tại: **complete**
- File đã đụng:
  - `apps/ios-swift/GenGate/Features/Location/LocationPlaceholderView.swift`
- Test-verify:
  - `cd apps/ios-swift && swift build` → ✅ (`Build complete! (2.05s)`)
...
- Git mốc gần nhất:
  ...
  - working tree hiện tại: dirty (workflow docs đang cập nhật để sync batch425)
- Blocker nếu có:
  - none.
- Bước kế tiếp:
  - mở batch426 với 1 micro-slice product seam (ưu tiên notifications/location/feed parity follow-up) + giữ verify tối thiểu `make test-friendships`.
