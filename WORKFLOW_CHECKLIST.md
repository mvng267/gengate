...
- Scope hiện tại: direct-message shell — thêm first-unread jump status/read-state quick-copy marker để copy deterministic first-unread apply result trên web+iOS.
...
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `a690ecf` — `batch281: add first-unread jump quick-copy markers on web and ios`
  - working tree hiện tại: sạch
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch282 với 1 slice hẹp direct-message shell: thêm guard/status khi first-unread candidate không đổi (already_at_latest_or_no_unread) để testers đỡ nhầm kết quả no-op trên web+iOS.
