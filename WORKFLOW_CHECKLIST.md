...
- Batch workflow chính thức mới nhất trong checklist/status: **402 — iOS inbox shell now aligns read-cursor-triage quick-copy failure handling parity (`quick_copy_clipboard_unavailable` / `read_cursor_triage_quick_copy_failed`) with web.**
...
- Scope hiện tại: iOS inbox read-cursor quick-copy parity — align read-cursor-triage quick-copy failure handling trong `InboxPlaceholderView`.
- Trạng thái hiện tại: **complete**
- File đã đụng:
  - `apps/ios-swift/GenGate/Features/Inbox/InboxPlaceholderView.swift`
- Test-verify:
  - `cd apps/ios-swift && swift build` → ✅ (`Build complete! (15.12s)`)
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `a7e772d` — `batch402: harden ios read-cursor triage quick-copy failures`
  - working tree hiện tại: clean
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch403 với 1 slice hẹp kế tiếp theo seam MVP (ưu tiên iOS inbox first-unread-jump quick-copy failure token parity với web `first_unread_jump_quick_copy_failed`).
