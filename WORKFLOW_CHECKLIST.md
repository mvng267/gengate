...
- Batch workflow chính thức mới nhất trong checklist/status: **111 — iOS inbox row-tap focus-user picker slice is in verify**.
...
- Scope hiện tại: iOS inbox read-status UX hardening — quick set focus user from member summary row tap.
...
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `HEAD` (local batch111 slice)
  - working tree hiện tại: sạch (sau commit local, chưa push)
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch112 cho messaging friction tiếp theo (quick set read-cursor user + focus user đồng thời từ member row) trong iOS inbox shell
