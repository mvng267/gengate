...
- Batch workflow chính thức mới nhất trong checklist/status: **112 — iOS inbox row-tap focus+cursor picker slice is in verify**.
...
- Scope hiện tại: iOS inbox read-status UX hardening — row tap sets focus user + read-cursor target user together.
...
- Git mốc gần nhất:
  - commit gần nhất đã chốt: `HEAD` (local batch112 slice)
  - working tree hiện tại: sạch (sau commit local, chưa push)
- Blocker nếu có:
  - none
- Bước kế tiếp:
  - mở batch113 cho messaging friction tiếp theo (quick row action to set target message from member cursor) trong iOS inbox shell
